...
public class MongoDataDao extends CollectionScopedDao implements DataDao {
   ...
   @Override
   public DataDocument createData(final String collectionId, final DataDocument data) {
      Document document = new Document(data);
      dataCollection(collectionId).insertOne(document);

      String id = document.getObjectId(ID).toHexString();
      data.setId(id);
      return data;
   }
   ...
   public DataDocument patchData(final String collectionId, final String documentId, final DataDocument data) {
      ...
      Document patchedDocument = dataCollection(collectionId).findOneAndUpdate(idFilter(documentId), document, options);
