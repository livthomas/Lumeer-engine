...
public class MorphiaProjectDao extends OrganizationScopedDao implements ProjectDao {
   ...
   private Criteria createUserCriteria(Query<MorphiaProject> projectQuery, String user) {
      return projectQuery.criteria(MorphiaProject.PERMISSIONS + "." + MongoPermissions.USER_ROLES)
   ...
   private Query<MongoPermission> createPermissionQuery(String name) {
      return datastore.createQuery(databaseCollection(), MongoPermission.class)
                      .filter(MongoPermission.NAME, name)
                      .field(MongoPermission.ROLES).in(Collections.singleton(Role.READ.toString()));
   }

   private Criteria createGroupCriteria(Query<MorphiaProject> projectQuery, String group) {
      return projectQuery.criteria(MorphiaProject.PERMISSIONS + "." + MongoPermissions.GROUP_ROLES)
